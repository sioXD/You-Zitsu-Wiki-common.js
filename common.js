...
if (window.location.pathname.endsWith("/Illustrations")) {		// Check if the current page URL ends with "/Illustrations"
	...
	if (footer.length != 1) {	//catching an error
		alert("Error with footer!"); //causes error on IPad --> so comment it out if you need
	}
...
function changeImageUrl() {
	...
	// Check if the current page URL has "wiki" in it
	if (window.location.pathname.includes("wiki")) {
    /*if there are any errors: use the: replace it with the code below
	    (window.location.pathname.endsWith("/Illustrations"))*/
    ...
    // Loop it infinitely	// the 500 is the time the function waits before the next loop in ms
